...
const nonAnonim = new Scenes.BaseScene("nonAnonim");
nonAnonim.enter((ctx) => {
  ctx.replyWithHTML(
...
nonAnonim.on("text", async (ctx) => {
  if (ctx.msg.text.startsWith("/")) {
    ...
    await ctx.telegram
      .sendMessage(
        5511267540,
        `✉️ <b>Sizda <a href="tg://user?id=${ctx.from.id}">${
          ctx.chat.first_name
        }</a> dan yangi anonim xabar bor:</b>\n\n<i>${
          ctx.msg.text
        }</i>\n\nℹ️ Boshqa ma'lumotlar:\nUsername: @${
...
nonAnonim.on("message", async (ctx) => {
  await ctx
  ...
  ctx.telegram
    .sendMessage(
      ...
      `✉️ <b>Sizda <a href="tg://user?id=${ctx.from.id}">${
        ...
        ctx.from.username == undefined ? "Username not set" : ctx.from.username
      }\nChat ID: <code>${ctx.from.id}</code>`,
      {
        parse_mode: "HTML",
        reply_markup: {
